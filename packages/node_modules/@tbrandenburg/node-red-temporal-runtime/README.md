...
[![npm](https://img.shields.io/npm/v/%40tbrandenburg%2Fnode-red-temporal-runtime?label=npm)](https://www.npmjs.com/package/@tbrandenburg/node-red-temporal-runtime)
[![Node.js](https://img.shields.io/badge/node.js-%3E%3D22.9-339933?logo=node.js&logoColor=white)](https://nodejs.org/)
[![Temporal](https://img.shields.io/badge/Temporal-1.24.0-000000)](https://temporal.io/)
...
## Durable ingress spool for autonomous sources (issue #66)

Autonomous source nodes (scheduled Inject, MQTT receive, Dashboard actions,
and any other long-lived runtime source) are captured by `Capture` and
started as an `executeFlow` Workflow (see [How it works](#how-it-works)).
Between "Capture observed the emission" and "Temporal durably accepted the
Workflow start" there used to be a small window where a runner crash or a
temporarily unreachable Temporal server could silently lose that event -
this issue closes it with the smallest possible mechanism: a file-per-event
durable spool, no new database/queue/daemon.

For every autonomous ingress event, the exact `executeFlow` start input
(graph, node metadata, `flowVersion`, initial destination/message list, task
queue values, node execution timeout) is written durably - temp file,
fsync, atomic rename - to:

```
<userDir>/.node-red-temporal/ingress-pending/<uuid>.json
```

only THEN is `client.workflow.start()` attempted, using the stable Workflow
ID `ingress:<uuid>` and Temporal's `REJECT_DUPLICATE` reuse policy. The
record is deleted only once Temporal reports the start as accepted OR
already-started (a duplicate/already-started outcome is treated as
success, never a second Workflow). A start failure leaves the record
pending; it is retried a few times with a modest, bounded, in-process
backoff while the runner stays alive (no busy loop, no unbounded queue),
and unconditionally replayed again at the next runner startup - the disk
record is the sole source of truth once the in-process retries are
exhausted.

`<userDir>` is the SAME persistent Node-RED userDir already used for
context/credentials persistence (`--user-dir`); with no persistent userDir
configured, a fresh temp directory is used instead (matching this
runtime's existing "worker-local, ephemeral by default" model).

**What this does NOT claim:** atomicity with the original external source.
An MQTT broker ack, a Dashboard Socket.IO ack, or a physical device
disappearing are all still out of scope - this closes the runner-internal
loss window between Capture and Temporal, not universal exactly-once
ingress from every possible transport.

**Replay never rebuilds from the current flow.** A pending record always
replays the flow snapshot captured AT CAPTURE TIME. If the runner has since
been redeployed to a different flow, the old record still targets the old
`flowVersion` - the existing `FLOW_VERSION_MISMATCH` contract is the safe
failure for that case, not a silent execution against a newer flow.

Stock HTTP ingress ([issue #58](#http-ingress-issue-58)) already has its
own explicit request/response contract and idempotency mechanism and is
never routed through this spool.
...
![Node-RED editor A: Debug sidebar and node status relayed from runner B](docs/screenshots/nodered-debug-panel.png)

The same click, proxied through issue #61 to runner B, as a real Temporal
`executeFlow` Workflow in the Temporal Web UI:

![Temporal UI: the workflow executed on runner B for the same Inject click](docs/screenshots/temporal-ui-workflow.png)
