...
- Stock Node-RED `HTTP In`/`HTTP Response` node compatibility.
...
- **Runner ("B")** - a `node-red-temporal` process (this package) started
  ...
  the existing Temporal-backed Activity Worker.
...
module.exports = {
    ...
    // Keep A design-time only: flows must not also run locally.
...
flows are stopped/design-only (`runtimeState`/`runtimeFlowState`), then
...
Remove or comment out the `storageModule`/`credentialSecret`/`runtimeState`
...
module.exports = {
    // ... storageModule/credentialSecret/runtimeState as above ...
};
```
...
- **Hooks are process-global.** Run one Capture instance per Activity-worker
  process.
- **HTTP: outbound supported, inbound is not.**
  - ✓ outbound `http request` — an ordinary Activity, subject to the
    at-least-once semantics above.
  - ✗ inbound `http in → http response` — not implemented. These nodes
    depend on live, process-bound Express `req`/`res` connection objects
    that cannot be serialized into Temporal history or survive a worker
    restart, so they are outside the durable message boundary described
    below.
